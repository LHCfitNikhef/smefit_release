...
def build_uv_posterior(results):
    results["gWH"] = (
        (0 - 2j) * np.emath.sqrt(results.O3pl1) * np.emath.sqrt(results.O3pl2)
    ) / np.emath.sqrt(results.Oll)
    results["gWLf11"] = (
        (0 - 2j) * np.emath.sqrt(results.O3pl1) * np.emath.sqrt(results.Oll)
    ) / np.emath.sqrt(results.O3pl2)
    results["gWLf22"] = (
        (0 - 2j) * np.emath.sqrt(results.O3pl2) * np.emath.sqrt(results.Oll)
    ) / np.emath.sqrt(results.O3pl1)
    results["gWLf33"] = ((0 - 2j) * results.O3pl3 * np.emath.sqrt(results.Oll)) / (
        np.emath.sqrt(results.O3pl1) * np.emath.sqrt(results.O3pl2)
    )
    results["gWqf11"] = ((0 + 2j) * np.emath.sqrt(results.Oll) * results.OpqMi) / (
        np.emath.sqrt(results.O3pl1) * np.emath.sqrt(results.O3pl2)
    )
