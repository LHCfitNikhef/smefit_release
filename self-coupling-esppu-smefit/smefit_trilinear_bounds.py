...
result_dir = pathlib.Path("/data/theorie/jthoeve/smefit_release/results/esppu_paper")

runs_ind_lin = [
    "260324_jth_HLLHC_250GEV_LIN_IND_aggressive",
...
runs_ind_quad = [
    "260324_jth_HLLHC_250GEV_QUAD_IND_aggressive",
...
runs_glob_lin = [
    "260326_jth_HLLHC_250GEV_LIN_GLOB_aggressive",
...
runs_glob_quad = [
    "260325_jth_HLLHC_250GEV_QUAD_GLOB_aggressive_no_whitening",
    "260325_jth_LEP3_250GEV_QUAD_GLOB_aggressive",
    "260325_jth_FCCee_250GEV_QUAD_GLOB_aggressive",
    "260330_jth_LCF500_250GEV_LIN_GLOB_aggressive",  # update to QUAD once run is finished
...
def find_bounds(runs, hdi=True, th_scenario="aggressive"):
    ...
    for run in runs:
        ...
        with open(
            result_dir / run.replace("aggressive", th_scenario) / "fit_results.json",
            "r",
        ) as f:
        ...
        if not individual:
            samples_kappa = (
                corr_factor_Op * samples_op
                + corr_factor_OpBox * samples_OpBox
                + corr_factor_OpD * samples_OpD
            )
        else:
        ...
        if not hdi:
            ...
            low_Op, high_Op = az.hdi(samples_op, hdi_prob=0.68)
            low_kappa, high_kappa = az.hdi(samples_kappa, hdi_prob=0.68)
...
if compute_bounds:
    bounds_trilinear_aggressive = find_bounds(
        runs_all, hdi=True, th_scenario="aggressive"
    )
    bounds_trilinear_conservative = find_bounds(
        runs_all, hdi=True, th_scenario="conservative"
    )
    ...
    bounds_trilinear_aggressive = pd.read_csv(
        f"trilinear_bounds_aggressive.csv", index_col=[0, 1, 2]
    )
    bounds_trilinear_conservative = pd.read_csv(
        f"trilinear_bounds_conservative.csv", index_col=[0, 1, 2]
    )
...
for i, collider in enumerate(colliders):
    for key in offsets:
        ...
        try:
            row_aggressive = bounds_trilinear_aggressive.loc[
                (collider, order, fit_type)
            ]
            row_conservative = bounds_trilinear_conservative.loc[
                (collider, order, fit_type)
            ]
        except KeyError:
            continue
...
leg2 = ax.legend(
    handles=ci_handles, loc="center right", bbox_to_anchor=(0.98, 0.17), frameon=False
)
ax.add_artist(leg1)
