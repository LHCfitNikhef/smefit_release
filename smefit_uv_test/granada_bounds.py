...
mod_dict = {2: r"$\mathcal{S}$", 3: r"$\mathcal{S}_1$", 5: r"$\varphi$", 6: r"$\Xi$", 7: r"$\Xi_1$", 10: r"$\omega_1$",
            ...
            21: r"$\mathcal{B}$", 22:r"$\mathcal{B}_1$" , 23:r"$\mathcal{W}_1$" , 24:r"$\mathcal{W}_1$", 25:r"$\mathcal{G}$",
            27:r"$\mathcal{H}$", 33:r"$\mathcal{Q}_5$", 36:r"$\mathcal{Y}_5$", 37:r"$N$", 38:r"$E$" , 39:r"$\Delta_1$",
            40:r"$\Delta_3$", 41:r"$\Sigma$", 42:r"$\Sigma_1$", 43:r"$U$", 44:r"$D$", 45:r"$Q_1$", 47:r"$Q_y$", 48:r"$T_1$",
            49:r"$T_2$" }
...
latex_table_scalar = dict_to_latex_table(scalar_dict, mod_dict, uv_param_dict, "cl-heavy-scalar", "95\\% CL intervals of the heavy scalar fields UV couplings.")
latex_table_vboson = dict_to_latex_table(vector_boson_dict, mod_dict, uv_param_dict, "cl-heavy-vboson", "95\\% CL intervals of the heavy vector boson fields UV couplings.")
latex_table_vfermion = dict_to_latex_table(vector_fermion_dict, mod_dict, uv_param_dict, "cl-heavy-vfermion", "95\\% CL intervals of the heavy vector fermion fields UV couplings.")
...
def plot_uv_posterior(n_params, mod_nrs, EFT, name):
    ...
    for mod_nr in mod_nrs:
        ...
        if posterior_path_mod_lo.exists():
            ...
            for (key, samples_lo), (_, samples_nlo) in zip(posterior_lo.items(), posterior_nlo.items()):
                print(key)
                if key.startswith("O"):
                    continue
                else:
                    ...
                    ax.hist(samples_lo, bins='fd', density=True,
                                edgecolor="black",
                                alpha=0.4)

                    ax.hist(samples_nlo, bins='fd', density=True,
                            edgecolor="black",
                            alpha=0.4)
                    ...
                    ax.text(x_pos, y_pos, f"{uv_param_dict[key]}", fontsize=20, ha='left', va='top')
...
subprocess.run(["gs", "-dBATCH", "-dNOPAUSE", "-q", "-sDEVICE=pdfwrite",
                ...
                "table_scalar.pdf", "granada_posteriors_NHO_scalars.pdf", "granada_posteriors_HO_scalars.pdf",
                "table_vboson.pdf", "granada_posteriors_NHO_vbosons.pdf", "granada_posteriors_HO_vbosons.pdf",
                "table_vfermion.pdf", "granada_posteriors_NHO_vfermions.pdf", "granada_posteriors_HO_vfermions.pdf"])
