...
def confidence_ellipse(coeff1, coeff2, ax, facecolor="none", confidence_level=95, **kwargs):
    """
    ...
    **kwargs
    ...
    ell_radius_x = np.sqrt(chi2_qnt * eigval_sort[-1]) # 5.991 selects the 95 % boundary
...
def plot_contours(ax, posterior, ax_labels, coeff1, coeff2, kde, clr_idx, confidence_level=95):
    """
    ...
    if kde:
        sns.kdeplot(x=x_values, y=y_values, levels=[1 - confidence_level/100.0, 1.0], bw_adjust=1.2, ax=ax, fill=True, alpha=0.3, color=colors[clr_idx])
        sns.kdeplot(x=x_values, y=y_values, levels=[1 - confidence_level/100.0], bw_adjust=1.2, ax=ax, alpha=1, color=colors[clr_idx])
