...
all_operators = ["OpBox","OpD",'O3pQ3', 'O3pQ3prime', 'O3pl1', 'O3pl2', 'O3pl3', 'O3pq', 'O3pqprime', 'OWWW', 'Obp', 'Ocp', 'Oll', 'OpB', 'OpBox', 'OpD', 'OpG', 'OpQM', 'OpW', 'OpWB', 'Opdi', 'Ope', 'Opl1', 'Opl2', 'Opl3', 'Opmu', 'Opq1', 'Opq3', 'OpqMi', 'Opta', 'Opui', 'Otap', 'Otp','Omup']
#EW Scheme
Gf = 0.0000116638
MZ = 91.1876
MW = 80.385
...
def Higgs_gg(mq): #SM Resolved loop, collecting a mq outside to isolate the yukawa
    ...
    if t>1:
        ...
        rad = np.sqrt(1-t)
        return 3/2*t/mq*(1-1/4*(1-t)*(np.log((1+rad)/(1-rad))-complex(0,np.pi))**2)
...
def new_post(SMEFT_posteriors,smeft_coefficients,weights):
    ...
    if set(smeft_coefficients)<=set(SMEFT_posteriors.keys()):
        ...
        caller = stack[-3].name
        warnings.warn(f"Setting 0 posteriors due to absence of {set(smeft_coefficients)-set(SMEFT_posteriors.keys())} needed in {caller}")
...
def check_labels(SMEFT_posteriors):
    ...
    if len(missing_operators)>0:
        warnings.warn(f"Missing operators: {list(missing_operators)}, setting to 0 the effective couplings in which they are involved")
        return 0
    else:
        return 1
...
def zeta_A(SMEFT_posteriors):
    return 2*v2_over_LambdaNP2*new_post(SMEFT_posteriors,["OpB","OpWB","OpW"],[cw**2,-cw*sw,sw**2])
def zeta_W(SMEFT_posteriors):
    return 2*new_post(SMEFT_posteriors,["OpW"],[v2_over_LambdaNP2])
def zeta_Z(SMEFT_posteriors):
    return 2*v2_over_LambdaNP2*new_post(SMEFT_posteriors,["OpB","OpWB","OpW"],[sw**2,+cw*sw,cw**2])
def zeta_ZA(SMEFT_posteriors):
    return v2_over_LambdaNP2*new_post(SMEFT_posteriors,["OpB","OpWB","OpW"],[-2*sw*cw,sw**2-cw**2,2*cw*sw])  
  
#input 
def cH(SMEFT_posteriors):
    return new_post(SMEFT_posteriors,["OpBox","OpD"],[1,+0.25])*(2*v2_over_LambdaNP2) 

def cT(SMEFT_posteriors):
    return-1/2*new_post(SMEFT_posteriors,["OpD"],[v2_over_LambdaNP2]) 

def delta_v(SMEFT_posteriors):
    return 1/2*new_post(SMEFT_posteriors,["O3pl1","O3pl2","Oll"],[1,1,-1])*v2_over_LambdaNP2

def delta_g(SMEFT_posteriors):
    return -delta_v(SMEFT_posteriors)

def delta_gprime(SMEFT_posteriors):   
    return -delta_v(SMEFT_posteriors) - new_post(SMEFT_posteriors,["OpWB"],[cw/sw])*v2_over_LambdaNP2+cT(SMEFT_posteriors)/2/sw**2

def delta_e(SMEFT_posteriors):
    cT_ = cT(SMEFT_posteriors  )
    return -delta_v(SMEFT_posteriors)+cw**2/sw**2*cT_/2-new_post(SMEFT_posteriors,["OpWB"],[cw/sw])*v2_over_LambdaNP2

def eta_W(SMEFT_posteriors):
    return -1/2*cH(SMEFT_posteriors)-delta_v(SMEFT_posteriors)

def eta_Z(SMEFT_posteriors):
    return -0.5*cH(SMEFT_posteriors)-delta_v(SMEFT_posteriors)-cT(SMEFT_posteriors)
...
def delta_gl(SMEFT_posteriors,smeft1,smeft3,Q=-1,T3=-1/2):
    ...
    return (1/(T3-Q*sw**2)*(cw**2*dg*(T3+Q*sw**2)+sw**2*dgprime*(T3-Q-cw**2*Q)+new_post(SMEFT_posteriors,[smeft1,smeft3],[-1/2,T3])*v2_over_LambdaNP2+Q*sw*cw*d_ZAZ)+d_ZZ/2)
...
def delta_gr(SMEFT_posteriors,smeft,Qf=-1):
    ...
    d_ZZ = delta_ZZ(SMEFT_posteriors)
    d_ZAZ = delta_ZAZ(SMEFT_posteriors)
    return (-cw**2*dg+(1+cw**2)*dgprime+1/2/Qf/sw**2*new_post(SMEFT_posteriors,[smeft],[1])*v2_over_LambdaNP2+(d_ZZ/2-cw/sw*d_ZAZ))

#gW definition
def delta_gW(SMEFT_posteriors,smeft):
    return delta_g(SMEFT_posteriors) + new_post(SMEFT_posteriors,[smeft],[v2_over_LambdaNP2])


#WW and ZZ decay definitions
QQ=3*(-1/2+sw**2)**2+3*(1/2)**2+3*(sw**2)**2+2*3*(1/2-2/3*sw**2)**2+3*3*(-1/2+1/3*sw**2)**2+3*3*(1/3*sw**2)**2+3*2*(-2/3*sw**2)**2
Qqe=3*(-1/2+sw**2)*(-1)+3*(sw**2)*(-1)+2*3*(1/2-2/3.*sw**2)*2/3+3*3*(-1/2+1/3*sw**2)*(-1/3)+3*3*(1/3*sw**2)*(-1/3)+2*3*(-2/3*sw**2)*2/3
Qr=0.529 #sticking to peskin

def compute_CZ(SMEFT_posteriors):
    pref = v2_over_LambdaNP2/2
    el = -pref*new_post(SMEFT_posteriors,["Opl1","O3pl1"],[1,1]) #check sign of prefactor
    ...
    lrcharge = +sw**2
    nucharge = 1/2
    CZ =(3*uplcharge*(ul+cl)+3*downlcharge*(dl+sl+bl)+
         3*uprcharge*(ur+cr)+3*downrcharge*(dr+sr+br)+
         nucharge*(vel+vmul+vtaul)
         +llcharge*(el+mul+taul)+
         lrcharge*(er+mur+taur))/(QQ)    
    return CZ 

def compute_CW(SMEFT_posteriors):
    return v2_over_LambdaNP2*new_post(SMEFT_posteriors,["O3pl1","O3pl2","O3pl3","O3pqprime"],[1,1,1,6])/(1+1+1+3+3)
...
def deltaGammaZ(SMEFT_posteriors):
    ...
    d_ZZ = delta_ZZ(SMEFT_posteriors)
    d_ZAZ = delta_ZAZ(SMEFT_posteriors)
    return 2*cw**2*(1+2*Qr*sw**2)*dg+2*sw**2*(1-2*Qr*cw**2)*dgprime+d_ZZ+Qr*sw*cw*d_ZAZ+2*CZ
...
def delta_gHZZ(SMEFT_posteriors):
    ...
    return 1/2*(2*eta_Z_-2*dv-0.5*zeta_Z_-1.02*CZ+1.18*delta_gammaZ)
...
def delta_gHZA(SMEFT_posteriors):
    ...
    return 1/2*(290*zeta_ZA_-cH_-2*(1-3*sw**2)*dg+6*cw**2*dgprime+d_ZA+d_ZZ-2*dv)
...
def delta_gHWW(SMEFT_posteriors):
    ...
    return 1/2*(2*eta_W_-2*dv-0.75*zeta_W_-0.88*CW+1.06*delta_gammaW)
...
def delta_gHgg(SMEFT_posteriors):
    ...
    cH_ = cH(SMEFT_posteriors)
    dv = delta_v(SMEFT_posteriors)
    return -cH_/2-dv-SM_pref*v2_over_LambdaNP2*np.real((MT/vev*Higgs_tt+MB/vev*Higgs_bb+MC/vev*Higgs_cc)*((new_post(SMEFT_posteriors,["Otp"],[1/np.sqrt(2)])*Higgs_tt+new_post(SMEFT_posteriors,["Obp"],[1/np.sqrt(2)])*Higgs_bb+new_post(SMEFT_posteriors,["Ocp"],[1/np.sqrt(2)])*Higgs_cc)-(3*4*np.pi/alpha_s/vev)*new_post(SMEFT_posteriors,["OpG"],[1.])))/Higgsgg_SM_full
# TGC  
...
def delta_guL(SMEFT_posteriors,):
...
#W couplings
def delta_gWe(SMEFT_posteriors,):
...
def delta_gHff(SMEFT_posteriors,smeft,mf):
    cH_ = cH(SMEFT_posteriors)
    dv = delta_v(SMEFT_posteriors)
    return 0.5*(-cH_-new_post(SMEFT_posteriors,[smeft],[2])*v2_over_LambdaNP2/(np.sqrt(2)*mf/vev)-2*dv)
