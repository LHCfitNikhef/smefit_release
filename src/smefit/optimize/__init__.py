    ...
    def load_external_chi2(self, external_chi2):
        ...
        for class_name, module in external_chi2.items():
            ...
            if self.rge_dict is not None:
                # Check if dynamic scale in RGE options
                if self.rge_dict["obs_scale"] == "dynamic":
                    ...
                    # Check if dynamic scale also in external likelihood
                    scale = module["scale"]
                    if scale == "dynamic":
                        _logger.warning(
                        ...
                        chi2_ext = my_chi2_class(
                            coefficients=new_coeffs, rgemat=self.rgemat, **module
                        )
                    else:
                        _logger.info(
                        ...
                        new_coeffs = CoefficientManager.from_dict(operators_dict)
                        chi2_ext = my_chi2_class(
                            coefficients=new_coeffs, rgemat=rge_df.values, **module
                        )
